...
# Глобальный обработчик ошибок входящих параметров
# ошибка RequestValidationError вызывается FastAPI при сбое проверки запроса.
...
async def get_openapi_json(request: Request):
    return JSONResponse(get_openapi(
        title="tools4bizz marketplace API",
        version="1.0.0",
        description="**Апи для маркетплейса**",
        routes=app.routes
    ))
